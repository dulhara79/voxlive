...
export function useTranscription() {
  ...
  const [langStats,    setLangStats]    = useState({ 'en-US': 0, 'si-LK': 0, 'ta-LK': 0 });
  ...
  const openWebSocket = useCallback(() => {
    return new Promise((resolve, reject) => {
      ...
      ws.onerror = () => {
        clearTimeout(timeout);
        reject(new Error('WebSocket error — check backend URL and CORS settings'));
      };
      ...
      ws.onclose = () => {
        ...
        console.log(`[VoxLive] Reconnecting (${reconnectCount.current}/${MAX_RECONNECT})…`);
